# Добавление к проекту на Java пакетов разработки (JAR)

## Шаг 1. Определение необходимых пакетов

Поиск необходимых пакетов выполняем в репозетории `maven` используя запрос в поисковой системе яндекса `ya.ru`, например, для `gurux` запрос выглядит следующим образом:
"**gurux site:gurux jar site:repo1.maven.org**"
